from loudspeakers.Parameter import Parameter, sqrt
...
class Loudspeaker(object):
    def __init__(self, brand, model):
        ...
        # suspension whereas a higher value indicates that the suspension complies more easily.
        # The spider contributes to ~80% of the compliance and the surround ~20%.
        ...
        # Bl is a measurement of the motor strength. It represents the mechanical force developed by the current
    ...
    def compute_qts(self):
        computed_qts = (self.Qms * self.Qes) / (self.Qms + self.Qes)
        print("Calculated Qts = {} [{}], provided value = {} {}"
              .format(computed_qts.value, computed_qts.unit.get_in_SI(), self.Qts.value, self.Qts.unit.get_in_SI()))
        return computed_qts

    def compute_fs(self):
        computed_fs = 1.0 / (2.0 * np.pi * sqrt(self.Cms * self.Mms))
        print("Calculated Fs = {} [{}], provided value = {} {}"
              .format(computed_fs.value, computed_fs.unit.get_in_SI(), self.Fs.value, self.Fs.unit.symbol))
        return computed_fs
